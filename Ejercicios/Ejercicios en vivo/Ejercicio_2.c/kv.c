...
static int kv_evict1() {
  ...
  for (p = LRU.prev; p != &LRU; p = p->prev) {
    ...
    if (pthread_mutex_trylock(&Table[bin].lock))
      continue;
     ...
     * tabla), lo ignoramos
     */
    if (kv->list.next == NULL || kv->list.prev == NULL) {
      pthread_mutex_unlock(&Table[bin].lock);
      continue;
    }
...
struct kv *kv_lookup(int klen, const char *k) {
  ...
  pthread_mutex_lock(&Table[idx].lock);
  ...
  pthread_mutex_unlock(&Table[idx].lock);
...
static
struct kv *new_node(u64 hash, int idx, int klen, char *k, int vlen, char *v) {
